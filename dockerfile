...
RUN conda env create -f dlotter.yml

#SHELL ["conda", "run", "-n", "dlotter", "/bin/bash", "-c"]
...
RUN wget https://naturalearth.s3.amazonaws.com/10m_cultural/ne_10m_admin_0_boundary_lines_land.zip -P ${CARTOPY_DIR}
RUN unzip ${CARTOPY_DIR}/ne_10m_admin_0_boundary_lines_land.zip -d  ${NE_CULTURAL}
RUN rm ${CARTOPY_DIR}/*.zip
