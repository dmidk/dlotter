...
class grib2Read:
    ...
    def set_bools(self) -> None:
        ...
        self.search_cin = False
        self.found_cin = False
        if 'cape' in self.parameters:
            self.search_cape = True
            self.search_cin = True
    ...
    def read(self, args:argparse.Namespace, files_to_read:list) -> xr.Dataset:
        ...
        data = []
        for i,d in enumerate(args.directory.split(",")):
            Nt = len(files_to_read[i])
            Nt_coords = np.zeros(Nt, dtype=dt.datetime)

            grid_idx = 0 # Set to 0
            lats, lons = self.get_latlons(files_to_read[i][grid_idx])
            ...
            if self.search_cin: cin = np.full([Nt,lats.shape[0],lons.shape[1]], np.nan)
            ...
            for k,f in enumerate(files_to_read[i]):
                ...
                if self.search_cape and (shortName=='cin') and level==0 and \
                                        typeOfLevel=='heightAboveGround' and levelType=='sfc':
                    values = ec.codes_get_values(gid)
                    self.found_cin = True
                    cin[k,:,:] = values.reshape(Nj, Ni)
            ...
            if self.found_cin: ds_grib['cin'] = (['time', 'lat', 'lon'], cin )
...
class netcdf2read:
    ...
    def read(self, args:argparse.Namespace, files_to_read:list) -> xr.Dataset:
        ...
        data = []
        for i,d in enumerate(args.directory.split(",")):
            lats, lons = self.get_latlons(files_to_read[i])
            ...
            if len(list(ds_grib.data_vars)) == 0:
                raise SystemExit('No variables found.')
